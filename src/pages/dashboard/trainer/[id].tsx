      ...
      <div className="flex-1 p-6 bg-black text-white">
        ...
        <div className="bg-gray-800 shadow-md rounded-lg p-6 mb-8">
          ...
          <ul>
            {clients.length > 0 ? (
              clients.map((client) => (
                <li key={client.id} className="p-4 mb-4 border-b hover:bg-gray-700" style={{ borderBottom: "1px solid #00ff66" }}>
          ...
          <div className="grid gap-4">
            {unassignedClients.length > 0 ? (
              unassignedClients.map((client) => (
                <div key={client.id} className="p-6 border border-green-500 rounded-lg" style={{ backgroundColor: "#1a1a1a", boxShadow: "0px 4px 6px rgba(0, 0, 0, 0.1)" }}>
                  <div className="flex justify-between">
                      ...
                      onClick={async () => {
                        try {
                          const response = await fetch("/api/assign-client", {
                            ...
                            body: JSON.stringify({ clientId: client.id, trainerId: user.id }),
                          });

                          if (!response.ok) {
                            throw new Error(`Failed to assign client. Status: ${response.status}`);
                          }
                          ...
                          if (error instanceof Error) {
                            ...
                            setError("An error occurred while assigning the client.");
                          }
                        }
                      }}
